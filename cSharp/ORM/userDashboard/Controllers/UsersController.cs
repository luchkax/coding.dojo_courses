...
namespace userDashboard.Controllers
{
    public class UsersController : Controller
    {
        ...
        public UsersController(DashContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Index()
        {
            ...
            {   
                User exists = _context.Users.SingleOrDefault(user=>user.UserId == id);
                if(exists.UserLevel == "Admin")
                {
        ...
        {   
            if(ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    if(exists.UserLevel == "Admin")
                    {
                        //If user IS admin redirect him to ADMIN DASHBOARD
                    ...
                    //If user is not admin redirect him to simple DASHBOARD
                    else
                    {
                        var hasher = new PasswordHasher<User>();
                        if(hasher.VerifyHashedPassword(exists, exists.Password, loginUser.Password) == 0)
